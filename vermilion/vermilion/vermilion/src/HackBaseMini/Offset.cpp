...
bool AutoOffset::GetRendererInfo()
{
	...
	const bool requireGameUI = !IsTargetCS11();
	const bool requireVgui2 = !IsTargetCS11();
...
PVOID AutoOffset::EngineFuncs()
{
	...
	{
		// legacy fallback (CS 1.1 / protocol 46): use ScreenFade string reference
		PCHAR String = "ScreenFade";
		DWORD Address = (DWORD)FindMemoryClone(HwBase, HwEnd, String, strlen(String));
		if (Address)
			EnginePtr = (cl_enginefunc_t*)*(DWORD*)(FindReference(HwBase, HwEnd, Address) + 0x0D);
	}
